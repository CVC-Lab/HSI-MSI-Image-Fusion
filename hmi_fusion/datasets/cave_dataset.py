...
# Function to convert label map to one-hot encoding
def one_hot_encode(label_map, num_classes):
    one_hot_map = np.eye(num_classes)[label_map]
    return one_hot_map
...
def encode_segmap(mask):
    ...
    for ii, label in enumerate(get_seg_labels()):
        label_mask[np.where(np.all(mask == label, axis=-1))[:2]] = ii
    label_mask = label_mask.astype(int)
    num_classes = len(get_seg_labels())
    return label_mask, num_classes
...
def load_data(dataset_dir, mode, cl):
    ...
    for c in os.listdir(dataset_dir):
        ...
        segmentation_path = os.path.join(dataset_dir, "segmentation_maps")
        ...
        seg_mask = cv2.imread(os.path.join(
            segmentation_path, c + "_seg","SegmentationClass", 
            c[:-3] + "_RGB.png"), -1)[:, :, ::-1]
        
        label_map, num_classes = encode_segmap(seg_mask)
        # seg_map = one_hot_encode(label_map, num_classes)
        seg_map = label_map / num_classes
        seg_map = seg_map[:, :, None]
        
        ms_img = load_ms_img(ms_path=ms_path)
        rgb_img = cv2.imread(rgb_path, -1)
        # rgb_img = np.array(Image.open(rgb_path))
        data.append((c, ms_path, ms_img, rgb_img, laplacian_img, seg_map))
...
class CAVEDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        c, _, HR_HSI, HR_MSI, lz, seg_map = self.data[idx]
        # print(HR_HSI.max())
        # print(c)
        ...
        hr_hsi = hr_hsi.squeeze(0).float()/65535#/hr_hsi.max()
        hr_msi = hr_msi.squeeze(0).float()/255#hr_msi.max()
        lr_hsi = lr_hsi.squeeze(0).float()/65535#/lr_hsi.max()
        ...
        return lr_hsi, hr_msi, hr_hsi, seg_map/255, max_vals
...
class CAVEMaterialDataset(Dataset):
    def __init__(self, dataset_dir, cl, mode="train", sf=8, transform=ToTensor()) -> None:
        super().__init__()
    ...
    def __getitem__(self, idx):
        ...
        # c, y, z, x, lz
        # c, x_old, y, z, _, lz, idx
        return lr_hsi, hr_msi, hr_hsi, seg_map, max_vals