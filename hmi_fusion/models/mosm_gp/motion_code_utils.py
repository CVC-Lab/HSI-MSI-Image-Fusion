...
from mosm_gp.sparse_gp import *
from mosm_gp.utils import *
...
def classify_predict_helper(X_test, Y_test, kernel_params_all_motions, X_m, Z, mu_ms, A_ms, K_mm_invs, mode='dt'):
    """
    Classify by calculate distance between inducing (mean) values and 
    interpolated test values at inducing pts.
    """
    ...
    for k in range(num_motion):
        ...
        if mode == 'simple':
            ...
            mean, _ = q(X_test, X_m_k, kernel_params_all_motions[k], 
                        mu_ms[k], A_ms[k], K_mm_invs[k])
