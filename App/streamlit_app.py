...
def main_page():
    ...
    st.markdown(
        ...
        The graph Laplacian was chosen such that it defines the affinity between the correlation windows, using both spectral and spatial information. Conjugate gradient was used to update K and X.
        ...
        We implemented the paper on the Indian Pines dataset. We chose the corrected image of shape (145, 145, 200) as our initial HSI. We then denoised it to obtain the SRI (ground truth).  We performed band selection to select the 16 most informative bands of the HSI, based on the Minimum Noise Band Selection algorithm ([MNBS](https://ieeexplore.ieee.org/document/6812179)). The MSI was generated from the spectral responses obtained from the Landsat_TM5 sensor. In the simulation process, we calculated the wavelength of the selected 16 bands. For each of the 6 spectral responses (blue, green, red, near infrared, short wave infrared 1 and 2) we perform a spline interpolation from the band with the highest wavelength below the given band, and the band with the lowest wavelength above the given band, which gives us the resultant band. The 6 bands together gave the denoised MSI. We added noise of signal to noise ratio (SNR) 40 to produce the final MSI. The HSI was generated by first applying an initial blur determined by its center, and radius (or downsampling ratio), and then adding noise of SNR=30. We generated the HSI corresponding to 3 settings:
        1. (mis-registration: 0) We take the blur kernel of size 9x9 (downsampling ratio 4) centered at the origin (ground truth blur kernel).
