...
if __name__ == "__main__":
    ...
    # 1. Train model on the training set for each dataset using XGBoost
    clf_orig = XGBClassifier(use_label_encoder=False, eval_metric='mlogloss')
    clf_orig.fit(X_train_orig, y_train_orig)

    clf_fused = XGBClassifier(use_label_encoder=False, eval_metric='mlogloss')
    clf_fused.fit(X_train_fused, y_train_fused)

    clf_msi = XGBClassifier(use_label_encoder=False, eval_metric='mlogloss')
